...
function setupHost(
  ...
  const oauthDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ig-oauth-'));
...
const okFetch = () =>
  jest.fn(async () => ({
    ok: true,
    status: 200,
    json: async () => ({ id: '178', username: 'eldar' }),
  }));

describe('submitInstagram', () => {
  ...
  it('creates an account, fresh sync_state, and an encrypted token blob', async () => {
    ...
    const r = await submitInstagram({ token: 'IGQ-longtoken' }, ctx);
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.username).toBe('eldar');

    const accounts = await db.all(
      `SELECT id, identifier, display_name, credentials_blob_path AS p FROM accounts WHERE source='instagram'`,
    );
    expect(accounts).toHaveLength(1);
    expect(accounts[0].identifier).toBe('178');
    expect(accounts[0].display_name).toBe('eldar');
    ...
    expect(token.access_token).toBe('IGQ-longtoken');
    expect(token.ig_user_id).toBe('178');
    await db.close();
  });

  it('rejects an empty token without writing to the DB', async () => {
    const db = openTestDb();
    const { ctx } = setupHost(db);
    const r = await submitInstagram({ token: '' }, ctx);
