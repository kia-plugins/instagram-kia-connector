...
export interface UpsertAccountInput {
  source: string;
  /** UNIQUE(source, identifier) key — for Instagram the ig_user_id. */
  identifier: string;
  displayName: string;
 ...
 *
 * Re-adding an existing account must clear the whole cursor: a stale cursor
 * (often status='live' with a delta window from the old token) would keep the
 * scheduler in delta mode and silently skip a fresh sync.
 */
